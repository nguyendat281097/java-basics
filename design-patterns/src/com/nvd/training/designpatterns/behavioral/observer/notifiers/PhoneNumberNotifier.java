...
public class PhoneNumberNotifier extends AbstractObserver {
	
	public PhoneNumberNotifier(Subject subject) {
	...
	public void notify(Subject subject, Object... objects) {
		if (subject instanceof VideoData) {
			...
			System.out.printf(
					"Notify all subscribers via PHONE NUMBER with new data" + "\n\tName: %s" + "\n\tDescription: %s"
							+ "\n\tFile name: %s",
